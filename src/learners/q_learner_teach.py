...
class QLearner_Teach:
    def __init__(self, mac_t, mac_s, scheme, logger, args):
        ...
        self.params_KD = list(self.mac_s.parameters())
        ...
        self.optimiser_KD = RMSprop(params=self.params_KD, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            student_outs = self.mac_s.forward(batch,t=t)
            mac_s_out.append(student_outs)
        mac_s_out = th.stack(mac_s_out, dim=1)

        # KD-error
        KD_error = (mac_s_out - mac_out.clone().detach())[:,:-1]
        KD_error = (KD_error ** 2).sum(axis=(2,3))
        ...
        loss_KD.backward()
        grad_norm_KD = th.nn.utils.clip_grad_norm_(self.params_KD, self.args.grad_norm_clip)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("grad_norm_TD", grad_norm, t_env)
            self.logger.log_stat("grad_norm_KD", grad_norm_KD, t_env)
            mask_elems = mask.sum().item()
            self.logger.log_stat("td_error_abs", (masked_td_error.abs().sum().item()/mask_elems), t_env)
            self.logger.log_stat("kd_error_abs", (masked_KD_error.abs().sum().item()/mask_elems), t_env)
    ...
    def load_models(self, path):
        ...
        self.optimiser.load_state_dict(th.load("{}/opt_TD.th".format(path), map_location=lambda storage, loc: storage))
        self.optimiser_KD.load_state_dict(th.load("{}/opt_KD.th".format(path), map_location=lambda storage, loc: storage))

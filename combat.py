...
class Combat(gym.Env):
    ...
    def __init__(self, grid_shape=(15, 15), n_agents=5, n_opponents=5, init_health=3, kaitu_range=4, full_observable=False,
    ...
    def get_agent_obs_kaitu(self):
        ...
        for agent_i in range(self.n_agents):
            # team id , unique id, location, health, cooldown
            _agent_i_obs = np.zeros((6, 2*self.kaitu_range+1, 2*self.kaitu_range+1))
            hp = self.agent_health[agent_i]
            ...
            if hp > 0:
                ...
                pos = self.agent_pos[agent_i]
                for row in range(0, 2*self.kaitu_range+1):
                    for col in range(0, 2*self.kaitu_range+1):
                        if self.is_valid([row + (pos[0] - self.kaitu_range), col + (pos[1] - self.kaitu_range)]) and (
                                PRE_IDS['empty'] not in self._full_obs[row + (pos[0] - self.kaitu_range)][col + (pos[1] - self.kaitu_range)]):
                            x = self._full_obs[row + pos[0] - self.kaitu_range][col + pos[1] - self.kaitu_range]
                            _type = 1 if PRE_IDS['agent'] in x else -1
                            _id = int(x[1:]) - 1  # id
                            _agent_i_obs[0][row][col] = _type
                            _agent_i_obs[1][row][col] = _id
    ...
    def get_obs_kaitu_size(self):
        return (2*self.kaitu_range+1)**2 * 6
